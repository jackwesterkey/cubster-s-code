...
public class AmmoManager4 : MonoBehaviour
{
    public int maxAmmo = 12; // Maximum ammunition
    public float currentAmmo;   // Current ammunition (changed to float for fractional ammo)
    ...
    // Method to consume 1 unit of ammunition
    public void UseAmmo()
    {
        // Consume 1 unit of ammo per shot
        currentAmmo -= 1.0f;
    ...
    private void OnGUI()
    {
        ...
        int iconsToDisplay = Mathf.CeilToInt(currentAmmo); // No need to multiply by 10 since you're consuming whole units
        iconsToDisplay = Mathf.Min(iconsToDisplay, maxAmmo); // Limit the icons to the maximum ammo capacity
